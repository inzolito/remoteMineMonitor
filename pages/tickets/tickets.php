                ...
                <table id="ticketsTable" class="table table-hover">
                    ...
                    <tbody id="dataTickets">
                        ...
                        foreach ($records_to_display as $case_number => $record) {
                            ...
                            if (date('Y-m-d', strtotime($record['data'][7])) === date('Y-m-d')) {
                                $newRowClass = "new-row";
                            } else {
                                $newRowClass = "";
                            }
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Consulta sobre tickets nuevos cada x tiempo 

        setInterval(function() {
            $.ajax({
                url: 'pages/scripts/scriptTickets.php?t=' + new Date().getTime(),
                success: function(data) {
                    if (ultimoCaseNumber == data) {

                    } else {
                        carga_modulo_container2('tickets/tickets.php','Tickets')
                    }
                }
            });
        }, 10000);
